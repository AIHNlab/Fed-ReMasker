...
class CategoricalEmbed(nn.Module):
    ...
    def __init__(self, possible_values=3, embed_dim=64, norm_layer=None):

        super().__init__()
        self.embed_dim = embed_dim
        self.proj = nn.Embedding(possible_values, self.embed_dim)
    ...
    def forward(self, x):
        ...
        #   x = torch.cat((torch.sin(x), torch.cos(x + math.pi/2)), -1)
        x = self.norm(x)
        return x.view(B, 1, self.embed_dim)
...
class SimilarMissingnessBatchSampler(Sampler):
    def __init__(self, missing_ratios, batch_size, shuffle=False):
        self.missing_ratios = missing_ratios
        self.batch_size = batch_size
        self.shuffle = shuffle

        # Precompute sorted indices
        self.sorted_indices = torch.argsort(missing_ratios)

        # Chunk into batches
        self.batches = [
            self.sorted_indices[i:i + batch_size].tolist()
            for i in range(0, len(self.sorted_indices), batch_size)
        ]

    def __iter__(self):
        if self.shuffle:
            # Shuffle batch order but keep samples within batch similar
...
def get_args_parser():
    ...
    parser.add_argument('--categorical_vocab_sizes', nargs='*', type=int, default=[], metavar='N', help='number of values for categorical variables')
    parser.add_argument('--cat_features', nargs='*', type=int, default=[], metavar='N', help='categorical variables positions')
